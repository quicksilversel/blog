...
- During local testing the OAuth redirect came back to `https://null/...`, because the library couldn't infer its own base URL when sign-in is triggered from a server action under the current framework version. Setting the canonical URL explicitly via env (`AUTH_URL`) fixed it deterministically. Worth setting in production too rather than trusting host inference behind a proxy.
